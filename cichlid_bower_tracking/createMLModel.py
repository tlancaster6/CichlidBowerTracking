...
parser.add_argument('Type', type = str, choices = ['3DResnet', 'RCNN'], help = 'Choose between a 3D Resnet for classifying videos or an RCNN to identify fish')
...
subprocess.run(['python3', '-m', 'cichlid_bower_tracking.unit_scripts.download_data', 'Train' + args.Type], '--ModelID', args.ModelID)

pp_obj = PP(projectID = None, modelID = args.NewModelID)
pp_obj.createModel(args.type, args.ProjectIDs, args.GPU)

subprocess.run(['python3', '-m', 'cichlid_bower_tracking.unit_scripts.upload_data', 'Train' + args.Type], '--ModelID', args.ModelID)
