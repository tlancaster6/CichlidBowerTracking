	...
	def _decompressVideo(self):
		...
		for i in range(0, totalBlocks, self.workers):
			...
			for j in range(self.workers):
				...
				cmnd = ['conda', 'activate', 'CichlidBowerTracking', '&&', 'python', 'Utils/Decompress_block.py',
						self.args.Movie_file, str(self.framerate), str(min_time), str(max_time),
	...
	def _calculateHMM(self):
		...
		for i in range(0, self.height, self.workers):
			...
			for row in range(start_row, stop_row):
				cmnd = ['conda', 'activate', 'CichlidBowerTracking', '&&', 'python', 'Utils/HMM_row.py', '--Rowfile',
						self.output_directory + str(row) + '.npy'] + self.row_command_arguments
				cmnd = ' '.join(cmnd)
